...
- Dùng **Blend Trees** để mượt mà hóa chuyển động giữa các hướng.
- Có hiệu ứng hạt (Particle) báo mất máu khi bị Zombie tấn công.

### 4. Hệ thống Kẻ địch (Zombies)
- **AI Tự động:** Tích hợp AI Navigation để quái liên tục dò đường tìm đến vị trí người chơi.
- **VFX & Shader:** Tự viết Custom Shader để làm hiệu ứng Zombie chớp trắng (Hit Flash) khi trúng đạn và hiệu ứng tan biến dần (Dissolve) khi chết.

### 5. Hệ thống Vũ khí (Weapons)
- Cài đặt hệ thống 2+ loại súng với các thông số khác nhau (Sát thương, tốc độ bắn).
- Nút chuyển súng (Switch Button) linh hoạt.
- Tích hợp đầy đủ **Particle System** cho vỏ đạn văng, tia lửa súng (Muzzle flash), và hiệu ứng nổ khi đạn chạm mục tiêu.
...
- **Level 2:** Địa hình có dốc (Slopes), nhịp điệu dồn dập và xuất hiện **ZOMBIE KHỔNG LỒ (Boss)**.
...
### 2. Kỹ năng Hỗ Trợ Đỉnh Cao (Air Support Skill)
- Xây dựng một tính năng kỹ năng gọi máy bay ném bom (Air Support). Máy bay sẽ bay từ rìa màn hình, quét qua bản đồ và xả mưa đạn liên tục vào kẻ địch. Hệ thống này có thể được nâng cấp cường độ nhờ hệ thống Level.

### 3. Tích hợp Backend (Firebase & Google Sign-in)
- Đã cài đặt **Firebase SDK** và **Google Sign-In**.
- Game tự động lưu trữ lượng Vàng (Gold) của người chơi lên cơ sở dữ liệu đám mây **Firestore**, đảm bảo dữ liệu không bị mất và có thể đồng bộ trên nhiều thiết bị.

### 4. Boss Khổng Lồ Với AI Phức Tạp (Advanced Boss AI)
Boss ở Level 2 không chỉ to hơn mà được lập trình State Machine riêng biệt với bộ 3 kỹ năng:
